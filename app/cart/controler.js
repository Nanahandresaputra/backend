...
const update = async (req, res, next) => {
  try {
    ...
    const products = productFood.find({ _id: { $in: productIds } });
    let cartItems = items.map((item) => {
      let relatedProduct = products.find((product) => product.id.toString() === item.product._id);
      return {
    ...
    await CartItem.deleteMany({ user: req.usser._id });
